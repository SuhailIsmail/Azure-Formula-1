...
race_results_df = spark.read.format("delta").load(f'{presentation_folder}/race_results').filter(f"file_date = '{v_file_date}'")
...
driver_stand = race_results.groupBy('race_year','driver_name','driver_nationality')\
...
driver_rank = Window.partitionBy('race_year').orderBy(desc('total_points'),
                                                      desc('wins'))
final_df = driver_stand.withColumn('rank', rank().over(driver_rank))

# COMMAND ----------

merge_condition = "tgt.race_year = src.race_year AND tgt.driver_name = src.driver_name "
merge_delta(final_df,'f1_presentation','driver_standings',presentation_folder, merge_condition,'race_year')